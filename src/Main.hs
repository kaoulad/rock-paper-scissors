...
type Model = (String, Bool, Picture, String, Int)
...
draw rock paper scissors (s, b, w, z, k) = if b then pure (Pictures [textPic s, rockPic rock, paperPic paper, scissorsPic scissors, picCPU w, label1, label2, label3, score z]) else pure (Pictures [textPic s, rockPic rock, paperPic paper, scissorsPic scissors, label1, label2, label3, score z])
        where textPic t = Scale 0.5 0.5 (Translate (-700) 410 (Text t))
              ...
              score m = Color blue (Scale 0.2 0.2 (Translate (50) 1050 (Text m)))
...
inputHandler rock paper scissors (EventKey (Char 'r') Down _ _) (t,b,w,z,k) = do                               
                                                                            tuple_ <- randomChoiceCPU (randomRIO (1,3)) rock paper scissors
                                                                            let score= whoWonScore "rock" (snd $ tuple_) k
                                                                            return (whoWon "rock" (snd $ tuple_), True, (fst $ tuple_), "Score:"++show score,score)

inputHandler rock paper scissors (EventKey (Char 'p') Down _ _) (t,b,w,z,k) = do
                                                                            tuple_ <- randomChoiceCPU (randomRIO (1,3)) rock paper scissors
                                                                            let score= whoWonScore "paper" (snd $ tuple_) k
                                                                            return (whoWon "paper" (snd $ tuple_), True, (fst $ tuple_), "Score:"++show score,score)

inputHandler rock paper scissors (EventKey (Char 's') Down _ _ ) (t,b,w,z,k) = do
                                                                            tuple_ <- randomChoiceCPU (randomRIO (1,3)) rock paper scissors
                                                                            let score= whoWonScore "scissors" (snd $ tuple_) k
                                                                            return (whoWon "scissors" (snd $ tuple_), True, (fst $ tuple_), "Score:"++show score, score)
inputHandler _ _ _ _ (t,b,w,z,k) = return (t,b,w,z,k)


updateFunc :: Float -> Model -> IO Model
updateFunc _ (t,b,w,z,k) = pure (t,b,w,z,k)
